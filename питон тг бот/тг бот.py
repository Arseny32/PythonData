...
def choose(message) -> None:
    ...
    try:
        if len(message.text.split()) == 2:  ### Готов
            if len(message.text.split()) == 2:
                for i in range(1, 3 + 1):
                    ...
                    cursor.execute(
                        f"SELECT * FROM teachers WHERE split_part(t_name, ' ', {i}) = '{mssg.capitalize()}';")
                ...
                for i in range(1, 2 + 1):
                    for j in range(i + 1, 3 + 1):
                        ...
                        print(i, j, ' ', mssg)  ##################################
                        cursor.execute(
                            f"SELECT * FROM teachers WHERE split_part(t_name, ' ', {i}) = '{mssg[0].capitalize()}' AND split_part(t_name, ' ', {j}) = '{mssg[1].capitalize()}';")
                        teacher = list(cursor.fetchall())
                        if teacher: break
                    if teacher: break
                ...
                teacher = list(cursor.fetchall())
        if len(message.text.split()) == 1: bot.send_message(message.chat.id,
                                                            text="Имя препода необходимо ввести после '/teacher'")
        if not teacher and len(message.text.split()) > 1: bot.send_message(message.chat.id, text="Такого препода нет")
        if len(message.text.split()) == 1: bot.send_message(message.chat.id,
                                                            text="Имя преподавателя необходимо ввести после '/teacher'")
        if not teacher and len(message.text.split()) > 1: bot.send_message(message.chat.id,
                                                                           text="Такого преподавателя нет")
        ...
        if len(teacher) == 1:
            ...
            t_name = teacher[0][1]
            bot.send_message(message.chat.id,
                             text=output(ratings=[teacher[0][1:-2][0], teacher[0][1:-2][1]], stage=stage),
    ...
    try:
        if message.text == 'Написать отзыв':  ### Готов
            if message.text == 'Написать отзыв':
                bot.send_message(message.chat.id,
                                 text='Введите отзыв как на примере (оценка не должна быть выше 5):\n4.9 Лучший препод')
...
else: bot.reply_to(message, text='Вы ввели фигню:' + message.text,
                   reply_markup=keyboard_f(tb, num=num, stage="TEACHER_CHOOSING"))
...
def handle_rate(message) -> None:  ### Готов (?)
    def handle_rate(message) -> None:
        ...
        if not (rate[0].isalpha()):
            if float(rate[0]) > 5.0:
                bot.send_message(message.chat.id,
                                 text=f"Оценка не должна превышать 5")  #####################################################################################################
                bot.send_message(message.chat.id, text=f"Оценка не должна превышать 5")
                bot.register_next_step_handler(message, handle_rate)
            else:
                try:
                    cursor.execute(
                        f"INSERT INTO ratings(t_name, rate_txt, rate) VALUES('{t_name}', '{rate[1]}', {rate[0]});")
                    conn.commit()
...
def goon_btn(call):
    ...
    bot.edit_message_text(chat_id=call.message.chat.id,
                          ...
                          text=output(num=num, t_name=t_name, ratings=get_ratings_ofone(tb, cursor, t_name),
                                      stage="TEACHER_BUTTON"),
                          reply_markup=keyboard_f(tb, num=num, max_num=len(get_ratings_ofone(tb, cursor, t_name)),
                                                  stage="TEACHER_LOOK"))
